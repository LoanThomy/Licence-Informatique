...
const requestDefibrillateurs = axios.get(urlDefibrillateurs);
requestDefibrillateurs.then(afficherDefibrillateurs);
...
function afficherDefibrillateurs(reponseAjax){
    const data = reponseAjax.data;
    const data_record = data.records;

    for (let cpt = 0; cpt < data_record.length; cpt++){
        afficherUnDefibrillateur(data_record[cpt]);
    }
}
